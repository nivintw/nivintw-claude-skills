...
description: >-
  ...
  issue" — or whenever development work is being planned, decomposed, or tracked in a
  GitHub repo. It defines a repeatable, robust workflow for using GitHub issues as the
  durable task ledger: capturing work as well-formed issues, triaging with a small
  status-label set (Projects optional), decomposing via native sub-issues, linking
  branches and PRs with "Closes #N", keeping issues current, and closing them
  deliberately. Prefer the GitHub MCP tools, falling back to the gh CLI. Make sure to
  reach for this skill whenever the user is organizing or tracking development tasks in
  GitHub, even when they don't say the word "issue".
---
