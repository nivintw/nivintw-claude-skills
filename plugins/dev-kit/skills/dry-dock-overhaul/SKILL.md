...
description: >-
  Use when the user asks for a "dry dock overhaul", to "audit the whole repo", "deep audit
  this codebase", or wants an exhaustive, occasional health check of an entire repository
  rather than a diff. Reads and judges every tracked source file, discovers and runs whatever
  "10,000-foot" communication/UX questions matter for this repo (docs-site UX, test
  architecture, naming, and the like), and orchestrates /dev-kit:review-pr (Mode C),
  /dev-kit:generate-docs, and /dev-kit:pre-public-hardening alongside those net-new
  dimensions. Produces one ephemeral, severity-ranked report — nothing it newly finds is
  auto-applied (generate-docs's native writes excepted). Always human-triggered. The most
  expensive skill in the marketplace by design — reach for it rarely, and on purpose.
---
