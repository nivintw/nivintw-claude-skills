...
run_cmd wtfis "wtfis ll gco git python" 0.5
pause 1.8
end_rec wtfis
...
start_rec fco 92 22
run_cmd fco "cd demo" 0.3
pause 0.6
run_cmd fco "fco" 0.3
pause 2.0 # fzf opens with log preview
type_human fco "login"
pause 1.6 # filter to feature/login (animated)
key fco Enter
pause 1.4 # select → checkout
run_cmd fco "git branch --show-current" 0.4
pause 1.4
end_rec fco

# ---- fif: live ripgrep search (interactive fzf, reloads per keystroke) -------
start_rec fif 92 22
run_cmd fif "cd demo" 0.3
pause 0.6
run_cmd fif "fif" 0.3
pause 2.0
type_human fif "log"
pause 1.2 # narrow as you type …
type_human fif "in"
pause 1.8 # … now "login"
key fif Escape
pause 1.0 # dismiss (Enter would open $EDITOR)
end_rec fif

# ---- gs-all: git status across every repo in a tree --------------------------
start_rec gs-all 92 20
run_cmd gs-all "cd multi" 0.3
pause 0.6
run_cmd gs-all "gs-all" 0.4
pause 2.2
end_rec gs-all

# ---- pyclean --dry-run: preview python cache cleanup -------------------------
start_rec pyclean 92 18
run_cmd pyclean "cd pyproj" 0.3
pause 0.6
run_cmd pyclean "pyclean --dry-run" 0.4
pause 2.2
end_rec pyclean
